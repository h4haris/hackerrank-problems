...
// give budget. Given price lists for keyboards and USB drives and a budget, find the cost to buy them. If it 
// is not possible to buy both items, return -1.
...
class Solution {
    ...
    static int getMoneySpent(int[] keyboards, int[] drives, int b) {
         ...
         */
        int[] priceCombinations = keyboards.SelectMany(k => drives.Select(d => k+d))
                                .OrderBy(p=>p)
                                .ToArray();
        
        for(int i = priceCombinations.Length - 1; i >= 0; i--)
        {
            if(priceCombinations[i] == b)
                return b;
            else if(priceCombinations[i] < b)
                return priceCombinations[i];
        }
