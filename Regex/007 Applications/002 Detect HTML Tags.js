...
////?    There may be 1 or more spaces before or after a tag name: < p >This is also a paragraph</>
...
var regexPattern = /<(?<TAG>\w+)(?:\s+(?<ATTRIBS>[^>]*))?>/gi

function processData(input) {
    const tags = new Set();

    while (tagMatch = regexPattern.exec(input)) {
        tags.add(tagMatch.groups.TAG.trim())
    }
