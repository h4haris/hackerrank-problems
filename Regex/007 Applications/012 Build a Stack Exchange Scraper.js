...
var regexPattern = /id=\"question-summary-\d+\"/gi

var regexPatternQues = /\/questions\/(?<Identifier>\d+)\/.*\>(?<Question>.+)\<\/a(?:.|\n)*relativetime"\>(?<Time>.+)</i

function processData(input) {
    const questions = input.split(regexPattern);

    questions.forEach(ques => {
        var match = regexPatternQues.exec(ques);

        if(match) {
            let gp = match.groups;
            if(gp) console.log(gp.Identifier.trim() + ';' + gp.Question.trim()+ ';' + gp.Time.trim())
        }
    });
}

processData(`<div class="question-summary" id="question-summary-80407">
<div class="statscontainer">
    <div class="statsarrow"></div>
    <div class="stats">
        <div class="vote">
