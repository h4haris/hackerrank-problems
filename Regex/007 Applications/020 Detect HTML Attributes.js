...
var regexPatternAttrib = /(\w+)=["'][^"']*["']/gi

function processData(input) {
    var results = []

    while (match = regexPatternTag.exec(input)) {
        let groups = match.groups
        var result = groups.TAG.trim() + ':';
        console.log(groups)
        if(groups.ATTRIBS) {
            var attribs = []
            // while (match = regexPatternAttrib.exec(input)){
            //     console.log(groups.ATTRIBS)
            // }
